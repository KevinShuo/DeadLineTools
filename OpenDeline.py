...
GpuList = []
CpuList = []
iniPath = r'Y:\LUXSERVER\DATA_BASE\02_TD\IT\DeadLineTools\SetUp.ini'
...
class time_class(QWidget):
    def __init__(self):
        super(time_class, self).__init__()
        while True:
            time.sleep(1)
            timea = datetime.datetime.today()
            hour = timea.strftime("%H%M")
            print(hour)
            if hour == "1855":
                self.setWindowFlags(Qt.WindowStaysOnTopHint)
                QMessageBox.information(self, "温馨提示",
                                        "离下班还有5分钟，为了给渲染节约内存请记得临走之前关闭工程文件！！！",
                                        QMessageBox.Ok)
                break
